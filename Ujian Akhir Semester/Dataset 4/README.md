...
Dataset       : `Sales of Shampoo over Three Years`
...
* KNIME workflow    : `UAS_Sales_of_Shampoo.knwf`
* File dataset  : Digunakan `sales-of-shampoo-over-a-three-ye.csv`
* Deskripsi dataset : Merupakan data penjualan shampoo dalam 3 tahun
...
Kolom pada data masih menggunakan nama default, yang memiliki nama `Month` yang sama seperti tipe data di SQL, dan kolom `Monthly beer production` memiliki whitespaces. Maka perlu direname kolomnya dengan node `Column Rename`.
...
Lalu dilakukan proses memasukkan variasi timeseries, yang berada di metanode `Extract date-time attributes`. Pada metanode tersebut, hanya terdapat `1` node `Spark SQL Query`, node untuk mengconvert tanggal dengan query SQL. Format tanggal di file kurang jelas, sehingga perlu saya asumsikan sendiri bahwa 1 digit pertama dari date field pada file adalah untuk tahun, dan sisanya 3 huruf pertama bulan. Sehingga untuk menjadi format tahun yang umum, digit pertama ditambah dengan int 2000 (3 tahun dimulai dari tahun 2001). Sementara untuk bulan harus dicari dari `substring` dan `when` untuk melakukan mapping satu-satu sesuai string yang cocok, dan hasilnya adalah berupa digit bulan bertipe int.
...
Berikutnya adalah proses pada metanode `Aggregations and time series`. Pada metanode ini akan ada `3` pecahan proses, untuk mencari rata-rata nilai pada kolom `daily_sales`. Sebelumnya, karena data digunakan berkali-kali, agar cepat dilakukan caching dengan node `Persist Spark DataFrame/RDD` ke dalam memory.
